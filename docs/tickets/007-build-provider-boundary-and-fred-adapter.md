...
## Acceptance Criteria
- [ ] A provider interface supports searching or resolving series metadata and fetching observations by series id and date range.
- [ ] The FRED adapter maps observations into normalized time-series points with date, value, unit, frequency, source, and fetched-at metadata.
- [ ] Missing API keys or provider failures fall back to clear errors or fixture mode without breaking the dashboard APIs.
- [ ] Unit tests mock FRED responses, empty series, invalid series ids, and network errors.
