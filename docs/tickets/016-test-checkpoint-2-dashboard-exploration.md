...
## Acceptance Criteria
- [x] Open the dashboard; Growth, Inflation, Policy / Liquidity, and Market render with 2-4 widgets each and no console errors.
- [x] Verify each visible value includes a unit and an observation date in the accepted date format.
- [x] Open indicator details from at least three widget types; metadata, source, and historical series are visible.
- [x] Simulate an API error for one widget; the dashboard remains usable and shows a factual widget-level error.
- [x] Confirm the dashboard uses warm light styling only and no spinning loaders.
