...
## Acceptance Criteria
- [x] A server-side agent entry point can receive a chat message and return a structured assistant response.
- [x] Agent tools wrap existing backend APIs for indicator search, widget data lookup, time-series retrieval, and series comparison.
- [x] Assistant responses include source, unit, and observation-date context when citing numeric data.
- [x] The agent integration has tests or mocks for tool calls, tool errors, and no-result responses.
